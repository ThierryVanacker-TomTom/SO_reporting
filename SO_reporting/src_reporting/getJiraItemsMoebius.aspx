...
url = "https://jira.tomtomgroup.com/rest/api/2/search?jql=" & Server.UrlEncode(jql) & "&fields=summary,issuetype,status,created,updated,duedate,resolutiondate,customfield_11860,fixVersions,components,customfield_19662,labels,resolution&maxResults=-1"
'response.write (url)
		...
		for each field_in_issue in issue
			...
			if (field_in_issue.Key.tostring = "fields" ) then
				for each field in field_in_issue.value 'loop though the fields within ONE issue
					...
					if (field.key.tostring = "labels") then
					if (field.value is nothing) then
						xml = xml & "<labels>" & "" & "</labels>" & vbcrlf
					else
						tmp = "||"
						for each label_item in field.value
						'for each field_in_label_item in label_item
						'if (field_in_label_item.key.tostring = "name") then
							tmp = tmp & trim(label_item) & "||"
						'end if
						'next
						next
						if tmp = "||" then tmp = ""
						xml = xml & "<labels>" & xmlsafe(tmp) & "</labels>" & vbcrlf
					end if
					end if
					...
					if (field.value is nothing) then
						...
						'if (field_in_sprintinfo_item.key.tostring = "name") then
