...
url = "https://jira.tomtomgroup.com/rest/api/2/search?jql=" & Server.UrlEncode(jql) & "&fields=summary,issuetype,status,created,updated,duedate,resolution,resolutiondate,customfield_11860,fixVersions,components,customfield_19662,resolution,customfield_16761&maxResults=-1"
'response.write (url)
		...
		for each field_in_issue in issue
			...
			if (field_in_issue.Key.tostring = "fields" ) then
				for each field in field_in_issue.value 'loop though the fields within ONE issue
					...
					if (field.key.tostring = "customfield_16761") then
					if (field.value is nothing) then
						xml = xml & "<team>" & "" & "</team>" & vbcrlf
					else
						xml = xml & "<team>" & xmlsafe("" & field.value.tostring) & "</team>" & vbcrlf
					end if
					end if
