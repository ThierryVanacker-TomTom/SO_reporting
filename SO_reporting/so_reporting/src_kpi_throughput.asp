...
jql = jql & " status changed to planned during (""" & from_ & """, """ & to_ & """)"
...
'set fso = nothing
	...
	For Each item_ In nodelist
		...
		For Each field_in_item_ In item_.ChildNodes
			...
			' next
			' end if
			'check if planned in this month
			if field_in_item_.BaseName = "transitions_history" then
			    ...
			    End If
			    Next
			    If bln2 = True Then ' it is set to planned in this month
				'do nothing
			    Else
				'not a good item
				'bln = False
...
'	response.write ("<br>" & replace(xml, "<" , "["))
	...
	i = 0
	j = 0
	response.write "<style='font-family:verdana;font-size:12pt;><b>Throughput</b>"
	...
	response.write"<br><style='font-family:verdana;font-size:10pt;>planned in month XX. Review for dates set to ""Planned"" vs ""To Review"" (also inclue the date assignee was set) ."
	...
	response.write "<td>" & "date set to ToReview"   & "</td>"
	response.write "<td>" & "date set to Planned"   & "</td>"
	response.write "<td>" & "assignee overview"   & "</td>"
	...
	For Each item_ In nodelist
		bln = true
		if bln = true then
		'ALG use this function to check that the item has a resolution of declined
			...
			response.write "<td>" & toDD_MM_YYYY(getSettoReview(getFieldValue(item_,"transitions_history"))) & "</td>"
			response.write "<td>" & toDD_MM_YYYY(getSettoPlanned(getFieldValue(item_,"transitions_history"))) & "</td>"
			response.write "<td>" & replace(getFieldValue(item_,"assignee_history"), "@@", "<br>") & "</td>"
...
toDate = s
if toDate <> "" then
	toDate = mid(s,7,2) & "/" & mid(s,5,2) & "/" & mid(s,1,4)
end if
end function
...
function getSettoReview(allstates)
getSettoReview = "00000000"
dim arr
dim arr2
dim a
...
if arr(a) <> "" then
	arr2 = split(arr(a), "||")
	if arr2(2) = "To Review" then
	getSettoReview = arr2(0)
	end if
end if
next
if getSettoReview = "00000000" then getSettoReview = ""
end function
...
'response.write "https://soreporting.azurewebsites.net/so_reporting/getJiraitems.aspx?project=" & project & "&yearmonth=" & yearmonth & "&rnd=" & rnd & ""
randomize timer
